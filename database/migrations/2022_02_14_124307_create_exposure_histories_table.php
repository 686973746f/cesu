...
class CreateExposureHistoriesTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('exposure_histories', function (Blueprint $table) {
            ...
            $table->foreignId('primarycc_id')->nullable()->constrained('records')->onDelete('cascade');
            $table->tinyInteger('is_secondarycc')->default(0);
            $table->foreignId('secondarycc_id')->nullable()->constrained('records')->onDelete('cascade');
            $table->tinyInteger('is_tertiarycc')->default(0);
            $table->foreignId('tertiarycc_id')->nullable()->constrained('records')->onDelete('cascade');
