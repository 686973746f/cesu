...
class User extends Authenticatable implements MustVerifyEmail
{
    ...
    public function canAccessTask() {
        $plist = $this->getPermissions();

        if(in_array('GLOBAL_ADMIN', $plist) || in_array('TASK_MEMBER', $plist)) {
            return true;
        }
        else {
            return false;
    ...
    public function canAccessDisaster() {
        $plist = $this->getPermissions();

        if(in_array('GLOBAL_ADMIN', $plist) || in_array('DISASTER_ADMIN', $plist) || in_array('DISASTER_ENCODER', $plist)) {
            return true;
        }
        else {
            return false;
    ...
    public function etclbhs() {
        return $this->belongsTo(DohFacility::class, 'etcl_bhs_id');
    }

    public function getBhsSwitchList() {
        if(!is_null($this->switch_bhs_list)) {
            return explode(",", $this->switch_bhs_list);
        }
        else {
            return [];
        }
