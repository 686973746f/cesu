...
class DailyAutoMW extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'automw:daily';
     ...
     * @var string
     */
    protected $description = 'Auto MW Daily';
    ...
    public function handle()
    {
        ...
        if(date('W') == 52 && date('m') == 01 || date('W') == 53 && date('m') == 01) {
            $lys = MorbidityWeek::where('year', date('Y', strtotime('-1 Year')))->first();

            if($lys) {
                $lyd = MorbidityWeek::findOrFail($lys->id);
            }
            else {
                $lydcreate = MorbidityWeek::create(['year' => date('Y', strtotime('-1 Year'))]);
