...
class Kernel extends ConsoleKernel
{
    ...
    protected function schedule(Schedule $schedule)
    {
        ...
        $schedule->exec('ps aux | grep "queue:work" | grep -v grep')->runInBackground()->everyMinute()->then(function ($output) use ($schedule) {
            if (empty($output)) {
                $schedule->command('queue:work --daemon')->runInBackground();
            }
        });
