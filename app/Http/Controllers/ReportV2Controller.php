...
class ReportV2Controller extends Controller
{
    ...
    public function accomplishment_index() {
        ...
        if(!(request()->input('choice'))) {
            ...
            $wtfchoice = request()->input('choice');
            if($wtfchoice == 'c1') {
    ...
    public function m2fhsis() {
        if(request()->input('year') && request()->input('month')) {
            ...
            foreach ($brgy as $b) {
                ...
                $sq = Forms::whereHas('records', function ($q) use ($b) {
                    $q->where('records.address_province', $b->city->province->provinceName)
                    ->where('records.address_city', $b->city->cityName)
                    ->where('records.address_brgy', $b->brgyName);
                })
                ...
                $tq = Forms::whereHas('records', function ($q) use ($b) {
                    $q->where('records.address_province', $b->city->province->provinceName)
                    ->where('records.address_city', $b->city->cityName)
                    ->where('records.address_brgy', $b->brgyName);
                })
    ...
    public function exportdb() {
        ...
        function queryGenerator($list) {
            foreach ($list->cursor() as $user) {
                yield $user;
            }
        }

        $sheets = new SheetCollection([
            'Forms' => queryGenerator($list),
        ]);
        ...
        ->export(storage_path('FORMS.xlsx'), function ($form) {
