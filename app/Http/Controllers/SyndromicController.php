...
class SyndromicController extends Controller
{
    ...
    public function downloadItrDocx($record_id) {
        ...
        $templateProcessor->setValue('s2c', ($d->alteredmentalstatus_onset == 1) ? '☐' : '☑');
        $templateProcessor->setValue('s3c', ($d->animalbite_onset == 1) ? '☐' : '☑');
        $templateProcessor->setValue('s4c', ($d->cough_onset == 1) ? '☐' : '☑');
        $templateProcessor->setValue('s5c', ($d->colds_onset == 1) ? '☐' : '☑');
        $templateProcessor->setValue('s6c', ($d->conjunctivitis_onset == 1) ? '☐' : '☑');
        $templateProcessor->setValue('s7c', ($d->anorexia_onset == 1) ? '☐' : '☑');
        $templateProcessor->setValue('s8c', ($d->fatigue_onset == 1) ? '☐' : '☑');
        $templateProcessor->setValue('s9c', ($d->fever_onset == 1) ? '☐' : '☑');
        $templateProcessor->setValue('s10c', ($d->headache_onset == 1) ? '☐' : '☑');
        $templateProcessor->setValue('s11c', ($d->jointpain_onset == 1) ? '☐' : '☑');
        $templateProcessor->setValue('s12c', ($d->jaundice_onset == 1) ? '☐' : '☑');
        $templateProcessor->setValue('s13c', ($d->lossofsmell_onset == 1) ? '☐' : '☑');
        $templateProcessor->setValue('s14c', ($d->lossoftaste_onset == 1) ? '☐' : '☑');
        $templateProcessor->setValue('s15c', ($d->musclepain_onset == 1) ? '☐' : '☑');
        $templateProcessor->setValue('s16c', ($d->nausea_onset == 1) ? '☐' : '☑');
        $templateProcessor->setValue('s17c', ($d->paralysis_onset == 1) ? '☐' : '☑');
        $templateProcessor->setValue('s18c', ($d->rash_onset == 1) ? '☐' : '☑');
        $templateProcessor->setValue('s19c', ($d->mouthsore_onset == 1) ? '☐' : '☑');
        $templateProcessor->setValue('s20c', ($d->sorethroat_onset == 1) ? '☐' : '☑');
        $templateProcessor->setValue('s21c', ($d->dyspnea_onset == 1) ? '☐' : '☑');
        $templateProcessor->setValue('s22c', ($d->vomiting_onset == 1) ? '☐' : '☑');
        $templateProcessor->setValue('s23c', ($d->weaknessofextremities_onset == 1) ? '☐' : '☑');
        $templateProcessor->setValue('s24c', ($d->other_symptoms_onset == 1) ? '☐' : '☑');
